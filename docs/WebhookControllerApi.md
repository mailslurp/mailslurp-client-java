...
<a id="createWebhookForAITransformer"></a>
# **createWebhookForAITransformer**
> WebhookDto createWebhookForAITransformer(transformerId, createWebhookOptions).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    WebhookControllerApi apiInstance = new WebhookControllerApi(defaultClient);
    UUID transformerId = UUID.randomUUID(); // UUID | 
    CreateWebhookOptions createWebhookOptions = new CreateWebhookOptions(); // CreateWebhookOptions | 
    try {
      WebhookDto result = apiInstance.createWebhookForAITransformer(transformerId, createWebhookOptions)
            .execute();
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling WebhookControllerApi#createWebhookForAITransformer");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
...
public class Example {
  public static void main(String[] args) {
    ...
    UUID aiTransformerId = UUID.randomUUID(); // UUID | AI Transformer ID
    try {
...
# **getAllWebhooks**
> PageWebhookProjection getAllWebhooks().page(page).size(size).sort(sort).searchFilter(searchFilter).since(since).inboxId(inboxId).aiTransformerId(aiTransformerId).phoneId(phoneId).before(before).health(health).eventType(eventType).url(url).eventTypeSource(eventTypeSource).includeAccountWide(includeAccountWide).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    OffsetDateTime since = OffsetDateTime.now(); // OffsetDateTime | Filter by created at after the given timestamp
    UUID inboxId = UUID.randomUUID(); // UUID | Filter by inboxId
    UUID aiTransformerId = UUID.randomUUID(); // UUID | Filter by aiTransformerId
    UUID phoneId = UUID.randomUUID(); // UUID | Filter by phoneId
    OffsetDateTime before = OffsetDateTime.now(); // OffsetDateTime | Filter by created at before the given timestamp
    String health = "HEALTHY"; // String | Filter by webhook health
    String eventType = "EMAIL_RECEIVED"; // String | Optional event type
    String url = "url_example"; // String | Optional url endpoint filter
    String eventTypeSource = "INBOX"; // String | Webhook source type category such as phone, inbox, aiTranformer
    Boolean includeAccountWide = true; // Boolean | Include account scope webhooks when passing phoneId, inboxId, or aiTransformerId filters
    try {
      PageWebhookProjection result = apiInstance.getAllWebhooks()
            ...
            .includeAccountWide(includeAccountWide)
            .execute();
      System.out.println(result);
    } catch (ApiException e) {
...
| **eventTypeSource** | **String**| Webhook source type category such as phone, inbox, aiTranformer | [optional] [enum: INBOX, PHONE, AI_TRANSFORMER] |
| **includeAccountWide** | **Boolean**| Include account scope webhooks when passing phoneId, inboxId, or aiTransformerId filters | [optional] |
...
> PageWebhookProjection getInboxWebhooksPaginated(inboxId).page(page).size(size).sort(sort).searchFilter(searchFilter).since(since).before(before).health(health).eventType(eventType).includeAccountWide(includeAccountWide).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    String health = "HEALTHY"; // String | Filter by webhook health
    String eventType = "EMAIL_RECEIVED"; // String | Optional event type
    Boolean includeAccountWide = true; // Boolean | Include account scope inbox webhooks
    try {
...
| **health** | **String**| Filter by webhook health | [optional] [enum: HEALTHY, UNHEALTHY] |
| **eventType** | **String**| Optional event type | [optional] [enum: EMAIL_RECEIVED, NEW_AI_TRANSFORM_RESULT, NEW_EMAIL, NEW_CONTACT, NEW_ATTACHMENT, EMAIL_OPENED, EMAIL_READ, DELIVERY_STATUS, BOUNCE, BOUNCE_RECIPIENT, NEW_SMS, NEW_GUEST_USER] |
| **includeAccountWide** | **Boolean**| Include account scope inbox webhooks | [optional] |
...
> PageWebhookProjection getPhoneNumberWebhooksPaginated(phoneId).page(page).size(size).sort(sort).since(since).before(before).eventType(eventType).searchFilter(searchFilter).health(health).includeAccountWide(includeAccountWide).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    String searchFilter = "searchFilter_example"; // String | Optional search filter
    String health = "HEALTHY"; // String | Filter by webhook health
    Boolean includeAccountWide = true; // Boolean | Include account scope phone webhooks
    try {
...
| **searchFilter** | **String**| Optional search filter | [optional] |
| **health** | **String**| Filter by webhook health | [optional] [enum: HEALTHY, UNHEALTHY] |
| **includeAccountWide** | **Boolean**| Include account scope phone webhooks | [optional] |
...
# **updateWebhook**
> WebhookDto updateWebhook(webhookId, createWebhookOptions).inboxId(inboxId).aiTransformerId(aiTransformerId).phoneNumberId(phoneNumberId).overrideAuth(overrideAuth).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    CreateWebhookOptions createWebhookOptions = new CreateWebhookOptions(); // CreateWebhookOptions | 
    UUID inboxId = UUID.randomUUID(); // UUID | 
    UUID aiTransformerId = UUID.randomUUID(); // UUID | 
    UUID phoneNumberId = UUID.randomUUID(); // UUID | 
