...
<a id="getExpirationDefaults"></a>
# **getExpirationDefaults**
> ExpirationDefaults getExpirationDefaults().execute();
...
<a id="getExpiredInboxByInboxId"></a>
# **getExpiredInboxByInboxId**
> ExpiredInboxDto getExpiredInboxByInboxId(inboxId).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    ExpiredControllerApi apiInstance = new ExpiredControllerApi(defaultClient);
    UUID inboxId = UUID.randomUUID(); // UUID | ID of inbox you want to retrieve (not the inbox ID)
    try {
...
<a id="getExpiredInboxRecord"></a>
# **getExpiredInboxRecord**
> ExpiredInboxDto getExpiredInboxRecord(expiredId).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    ExpiredControllerApi apiInstance = new ExpiredControllerApi(defaultClient);
    UUID expiredId = UUID.randomUUID(); // UUID | ID of the ExpiredInboxRecord you want to retrieve. This is different from the ID of the inbox you are interested in. See other methods for getting ExpiredInboxRecord for an inbox inboxId
    try {
...
<a id="getExpiredInboxes"></a>
# **getExpiredInboxes**
> PageExpiredInboxRecordProjection getExpiredInboxes().page(page).size(size).sort(sort).since(since).before(before).inboxId(inboxId).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    UUID inboxId = UUID.randomUUID(); // UUID | Optionally filter by inbox ID
    try {
      PageExpiredInboxRecordProjection result = apiInstance.getExpiredInboxes()
            .page(page)
            .size(size)
            .sort(sort)
            .since(since)
