...
<a id="extractAttachmentText"></a>
# **extractAttachmentText**
> ExtractAttachmentTextResult extractAttachmentText(attachmentId).extractAttachmentTextOptions(extractAttachmentTextOptions).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    AttachmentControllerApi apiInstance = new AttachmentControllerApi(defaultClient);
    String attachmentId = "attachmentId_example"; // String | ID of attachment
    ExtractAttachmentTextOptions extractAttachmentTextOptions = new ExtractAttachmentTextOptions(); // ExtractAttachmentTextOptions | 
    try {
      ExtractAttachmentTextResult result = apiInstance.extractAttachmentText(attachmentId)
            .extractAttachmentTextOptions(extractAttachmentTextOptions)
            .execute();
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling AttachmentControllerApi#extractAttachmentText");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
...
# **getAttachments**
> PageAttachmentEntity getAttachments().page(page).size(size).sort(sort).fileNameFilter(fileNameFilter).since(since).before(before).inboxId(inboxId).emailId(emailId).sentEmailId(sentEmailId).include(include).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    List<String> include = Arrays.asList(); // List<String> | Optional list of IDs to include in result
    try {
...
| **include** | [**List&lt;String&gt;**](String)| Optional list of IDs to include in result | [optional] |
...
# **uploadMultipartForm**
> List&lt;String&gt; uploadMultipartForm().contentId(contentId).contentType(contentType).filename(filename).contentTypeHeader(contentTypeHeader).xFilename(xFilename).xFilenameRaw(xFilenameRaw).xFilesize(xFilesize).importEmailIntoInboxMultipartRequest(importEmailIntoInboxMultipartRequest).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    ImportEmailIntoInboxMultipartRequest importEmailIntoInboxMultipartRequest = new ImportEmailIntoInboxMultipartRequest(); // ImportEmailIntoInboxMultipartRequest | 
    try {
