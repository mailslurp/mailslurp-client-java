...
public class DNSLookupResults {
  ...
  public DNSLookupResults results(@javax.annotation.Nonnull List<DNSLookupResult> results) {
  ...
  public void setResults(@javax.annotation.Nonnull List<DNSLookupResult> results) {
    this.results = results;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to DNSLookupResults
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException { if (true) { return; }
      if (jsonElement == null) {
        if (!DNSLookupResults.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DNSLookupResults.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DNSLookupResults` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayresults.size(); i++) {
        DNSLookupResult.validateJsonElement(jsonArrayresults.get(i));
      };
  }
