...
public class WebhookHeaders {
  ...
  public WebhookHeaders headers(@javax.annotation.Nonnull List<WebhookHeaderNameValue> headers) {
  ...
  public void setHeaders(@javax.annotation.Nonnull List<WebhookHeaderNameValue> headers) {
    this.headers = headers;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to WebhookHeaders
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException { if (true) { return; }
      if (jsonElement == null) {
        if (!WebhookHeaders.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!WebhookHeaders.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `WebhookHeaders` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayheaders.size(); i++) {
        WebhookHeaderNameValue.validateJsonElement(jsonArrayheaders.get(i));
      };
  }
