...
public class VerifyWebhookSignatureResults {
  ...
  public VerifyWebhookSignatureResults isValid(@javax.annotation.Nonnull Boolean isValid) {
   ...
   * @throws IOException if the JSON Element is invalid with respect to VerifyWebhookSignatureResults
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException { if (true) { return; }
      if (jsonElement == null) {
        if (!VerifyWebhookSignatureResults.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VerifyWebhookSignatureResults.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VerifyWebhookSignatureResults` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
